...
function createProjectCard() {
    ...
    for (let i = 0; i < student.projects.length; i++) {
      ...
      <div class="content-text">
        ...
        const startNumber = student.projects[i].startNumber || 1;

        for (let j = 0; j < student.projects[i].projectNumber; j++) {
            const projectNum = startNumber + j;
            // let childId = student.projects[i].folder + (j + 1);

            let childId = student.projects[i].folder + projectNum;
            let projectName = student.projects[i].projectNames[j] || `Project ${projectNum}`;
